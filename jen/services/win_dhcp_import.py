...
v5.28.0 (Q24, F0-F8) — validated against a real Windows Server 2019/2022
`Export-DhcpServer` file (`tests/fixtures/windows-dhcp-export-real.xml`,
two scopes, 57 reservations, per-reservation options): the v5.24.0
parser read every field via an XML-attribute accessor, but a real
export puts them in child elements, so it read nothing from one.
`_field()`/`_bool_field()`/`_int_field()` read either shape.
`tests/fixtures/windows-dhcp-export.xml` (authored from Microsoft's
documented shape, not a real export) still covers exclusions,
superscopes, policies, and options 121/249 — the real export contains
none of those, so treat that ground as reasoned-through rather than
confirmed against genuine output.
"""
