...
class TestSystemSettings:
    ...
    def test_save_ports(self, logged_in_client, monkeypatch):
        ...
        monkeypatch.setattr(threading, "Thread", lambda target, daemon: type("T", (), {"start": lambda self: None})())
        r = logged_in_client.post(
            "/settings/save-ports",
            data={
                "http_port": "5050",
...
class TestMfaModeAndNavLogoRoutesRegression:
    ...
    def test_save_mfa_mode_route_is_registered(self, logged_in_client):
        r = logged_in_client.post(
            "/settings/system/save-mfa-mode", data={"mfa_mode": "optional"}, follow_redirects=False
        )
    ...
    def test_save_mfa_mode_actually_persists(self, logged_in_client):
        ...
        r = logged_in_client.post(
            "/settings/system/save-mfa-mode", data={"mfa_mode": "required_admins"}, follow_redirects=True
        )
        assert r.status_code == 200
    ...
    def test_save_mfa_mode_rejects_invalid_value(self, logged_in_client):
        r = logged_in_client.post(
            "/settings/system/save-mfa-mode", data={"mfa_mode": "not_a_real_mode"}, follow_redirects=True
        )
        assert r.status_code == 200
    ...
    def test_save_mfa_mode_requires_admin(self, client, db):
        ...
        _restricted_client(client, db, allowed_subnets=None, role="viewer", username="mfamode_viewer1")
        r = client.post("/settings/system/save-mfa-mode", data={"mfa_mode": "optional"}, follow_redirects=True)
        assert r.status_code == 200
    ...
    def test_upload_nav_logo_requires_admin(self, client, db):
        ...
        _restricted_client(client, db, allowed_subnets=None, role="viewer", username="navlogo_viewer1")
        r = client.post(
            "/settings/upload-nav-logo",
