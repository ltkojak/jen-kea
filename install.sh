...
# ── .env value escaping ──────────────────────────────────────────────────────
# Docker Compose interpolates unquoted (and double-quoted) values in a
# .env file, so a password containing $, ${FOO}, a backtick, etc. would be
# mangled or silently emptied. Emit a value Compose reads verbatim:
#   - no single quote  -> wrap in single quotes (everything literal)
#   - has a single quote -> double-quote and escape \  $  "  ` so Compose's
#     interpolation leaves them alone ($$ is Compose's literal-$ escape)
# Used for EVERY generated .env value, not just the obvious secrets.
env_value() {
    local v="$1"
    if [[ "$v" != *"'"* ]]; then
        printf "'%s'" "$v"
...
# Values are quoted so Docker Compose does not interpolate \$ / \${..} in them.
