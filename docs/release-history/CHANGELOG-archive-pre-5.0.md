# Jen Changelog Archive — v4.4.24 and earlier

Entries prior to the 5.x series, moved out of the main CHANGELOG.md (v5.3.3) to keep that file scannable — it had grown to roughly 330KB covering every release since v1.0.0. Nothing here was deleted, only relocated; every entry below is byte-for-byte identical to how it originally appeared in CHANGELOG.md, verified programmatically before this split, not just assumed correct.

The 3.x series also has separate, more narrative release notes in this same directory (RELEASE-3.x.md files) — those are polished summaries written for that series' milestones, not a replacement for the entry-by-entry record kept here.
